    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (ticketsCtx) {
            const ticketsChart = new Chart(ticketsCtx, {
                type: 'doughnut',
                data: {
                    labels: @json($ticketStatusData ? array_keys($ticketStatusData) : []),
                    datasets: [{
                        data: @json($ticketStatusData ? array_values($ticketStatusData) : []),
                        backgroundColor: [
                            'rgba(245, 158, 11, 0.8)',
                            'rgba(59, 130, 246, 0.8)',
                            'rgba(16, 185, 129, 0.8)'
        ...
        if (equipmentsCtx) {
            const equipmentsChart = new Chart(equipmentsCtx, {
                type: 'bar',
                data: {
                    labels: @json($equipmentChartData ? array_keys($equipmentChartData) : []),
                    datasets: [{
                        label: 'Nombre d\'équipements',
                        data: @json($equipmentChartData ? array_values($equipmentChartData) : []),
        ...
        if (equipmentStatusCtx) {
            const equipmentStatusChart = new Chart(equipmentStatusCtx, {
                type: 'pie',
                data: {
                    labels: @json($equipmentStatusData ? array_keys($equipmentStatusData) : []),
                    datasets: [{
                        data: @json($equipmentStatusData ? array_values($equipmentStatusData) : []),
                        backgroundColor: [
                            'rgba(16, 185, 129, 0.8)',
                            'rgba(59, 130, 246, 0.8)',
                            'rgba(245, 158, 11, 0.8)',
